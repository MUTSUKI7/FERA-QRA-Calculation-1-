...
with st.sidebar:
    ...
    with st.expander("고급 설정 — 로브(lobe) 형태 가정치 (열복사·LSIR에 적용)"):
        ...
        st.caption(
            "**Jet fire**: 검증된 화염 기울기 상관식을 확보하지 못해(θ 보간 없음), 누출구(orifice) "
            "물리적 크기 기준 하한만 적용 — BACK_FRAC = CROSS_FRAC = 누출구 지름(Leak_Size) ÷ "
            "Jet_12.5 ('Jet fire는 점원에서 뿜어져 나오므로, 풍상측·좌우측에 실제로 존재하는 물리적 "
            "크기는 화염 전체가 아니라 누출구 자체 크기 정도'라는 논거). 최초 시도했던 화염 길이(L_flame) "
            "기준 방식은 L_flame이 R과 비례해 커져 홀이 클수록 오히려 풍상측이 넓어지는 모순이 발견돼 "
            "폐기했고, DNV GL Table 1(SEP=250 kW/m² 고정 계수)도 우리 데이터와 최대 89% 어긋나 채택하지 "
            "않았다."
        )
        use_physics_jet = st.checkbox(
            "Jet Fire에 누출구 크기 기반 기하학적 로브 형태 적용", value=True
        )
...
# ------------------------------------------------------------------
# Jet Fire의 BACK_FRAC/CROSS_FRAC — 누출구(orifice) 물리적 크기 기준 하한
#
# [수정 이력] 이전 버전은 BACK_FRAC=CROSS_FRAC=L_flame/R_hazard(화염 길이 기준)를 썼으나,
# 실제 데이터로 검증한 결과 명백한 결함이 발견되어 폐기함: 화염 길이(L_flame)는 방출량이
# 커질수록 위험거리(R)와 함께 "비례해서" 커지기 때문에, 그 비율(L_flame/R)이 홀 크기와 무관하게
# 항상 0.3~0.65 근처의 큰 값으로 나왔다. 그 결과 홀이 클수록 풍상측 거리가 오히려 더 커지는(최대
# 85m) 모순이 발생했는데, 이는 "Jet fire는 점원(누출구)에서 뿜어져 나오므로 풍상측엔 물리적
# 근거가 거의 없다"는 애초의 설계 의도와 정반대였다.
#
# 이를 바로잡아, 화염 "길이"가 아니라 누출구(orifice) 자체의 물리적 지름(D_hole)을 기준으로
# 삼는다: Jet fire가 점원에서 뿜어져 나온다는 논리를 살리려면, 풍상측·좌우측에 실제로 존재하는
# 물리적 크기는 화염 전체가 아니라 "누출이 시작되는 그 구멍의 크기" 정도여야 한다.
#     JET_FRAC = D_hole / R_hazard   (D_hole = Leak_Size_mm를 m로 환산한 값)
# D_hole(수 mm~150mm)은 R(수 m~수백 m)에 비해 항상 무시할 만큼 작아서, 실제로는 거의 모든
# 시나리오에서 수치 안전 하한(numeric_floor)에 의해 결정된다 — 이는 "Jet fire는 항상 가늘고
# 뾰족한 로브여야 한다"는 원래 취지를 정확히 만족시키는 결과다.
# (Jet fire 전용 검증된 화염 기울기 상관식은 확보하지 못해 theta 보간은 적용하지 않는다.)
# ------------------------------------------------------------------
def resolve_jet_fracs(use_physics, hole_diameter_m, hazard_radius_m,
                       fallback_back, fallback_cross, numeric_floor=0.02):
    """물리 모델 스위치가 켜져 있고 필요 데이터(누출구 지름, PHAST 위험거리)가 모두 있으면
    BACK_FRAC = CROSS_FRAC = D_hole / R_hazard 를 계산해 반환한다. 데이터가 없거나 스위치가
    꺼져 있으면 사이드바 슬라이더 값을 그대로 반환한다."""
    if not use_physics:
        return fallback_back, fallback_cross
    if hole_diameter_m is None or hazard_radius_m is None or hazard_radius_m <= 0:
        return fallback_back, fallback_cross
    if pd.isna(hole_diameter_m) or pd.isna(hazard_radius_m):
        return fallback_back, fallback_cross

    ratio = max(numeric_floor, min(1.0, hole_diameter_m / hazard_radius_m))
    return ratio, ratio
...
def render_thermal():
    ...
    hole_col_thermal = find_hole_size_col(
        phast, exclude_cols={"IS_ID", "Weather_Class"} | set(thermal_cols)
        | {"Pool_Diameter_m", "Pool_BurnRate_kgs", "Jet_FlameLength_m"}
    )
    phast["Hole_Diameter_m"] = (
        pd.to_numeric(phast[hole_col_thermal], errors="coerce") / 1000.0
        if hole_col_thermal is not None else np.nan
    )

    by_weather = phast.groupby(["IS_ID", "Weather_Class"])[
        ["Jet_12.5", "Jet_37.5", "Pool_12.5", "Pool_37.5",
         "Pool_Diameter_m", "Pool_BurnRate_kgs", "Jet_FlameLength_m", "Hole_Diameter_m"]].max().reset_index()
...
def compute_lsir_field(
    ...
    grid_res_m, back_frac, cross_frac, use_physics_pool=True,
):
            ...
            for wr in wind_rows:
                ...
                if cls["has_jet"]:
                    ...
                    bf_jet, cf_jet = resolve_jet_fracs(
                        use_physics_jet, hs / 1000.0, d.get("Jet_12.5"),
                        back_frac, cross_frac,
                    )
    ...
    with st.spinner("LSIR 계산 중..."):
        try:
            X, Y, risk_field, is_coords = compute_lsir_field(
                excel_bytes, grid_extent_x_m, grid_extent_y_m,
                coord_ref_extent_x_m, coord_ref_extent_y_m,
                grid_res_m, back_frac, cross_frac, use_physics_pool,
            )
        except Exception as e:
