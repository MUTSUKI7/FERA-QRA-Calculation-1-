...
with st.sidebar:
    ...
    with st.expander("고급 설정 — 로브(lobe) 형태 가정치 (열복사·LSIR에 적용)"):
        ...
        st.caption(
            "위 슬라이더는 Jet fire, 그리고 물리 데이터가 없는 Pool fire에 적용되는 고정값입니다. "
            "아래를 켜면 Pool fire는 AGA/Thomas(1963)의 화염 기울기(flame tilt, θ) 상관식으로 "
            "'모양'을, θ=0(원형)↔θ=90(=Pool_Diameter_m/2 ÷ Pool_12.5, 화염 자신의 물리적 크기 "
            "하한)의 두 물리적 경계값을 cos(θ)로 보간해 IS_ID·기상조건별 BACK_FRAC=CROSS_FRAC을 "
            "자동 계산합니다. (PHAST_Distances의 Pool_Diameter_m·Pool_BurnRate_kgs·Pool_12.5, "
            "Wind rose의 Wind_Speed_(m/s) 필요 — 없으면 그 시나리오는 슬라이더 값 또는 기하학적 "
            "하한만으로 대체됩니다.)"
        )
...
def render_thermal():
    ...
    by_weather = phast.groupby(["IS_ID", "Weather_Class"])[
        ["Jet_12.5", "Jet_37.5", "Pool_12.5", "Pool_37.5",
         "Pool_Diameter_m", "Pool_BurnRate_kgs"]].max().reset_index()
...
def compute_lsir_field(
    ...
    hole_col_phast = find_hole_size_col(
        phast_df, exclude_cols={id_col_phast, "Weather_Class"} | set(thermal_level_cols) | set(pool_physics_cols)
    )
    ...
    for _, row in phast_df.iterrows():
        ...
        phast[key] = {c: to_float_or_none(row[c]) for c in thermal_level_cols + pool_physics_cols}
        for c in ["Jet_12.5", "Jet_37.5", "Pool_12.5", "Pool_37.5", "Flash_LFL", "Exp_0.3", "Exp_0.5",
                  "Pool_Diameter_m", "Pool_BurnRate_kgs"]:
