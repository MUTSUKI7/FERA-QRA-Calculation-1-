...
try:
    ...
    HAS_CANVAS = True
    # 아주 최신 Streamlit(streamlit.elements.image.image_to_url이 아예 없는 경우)에서만
    # streamlit-drawable-canvas의 background_image 처리를 위한 임시 대체 함수를 끼워 넣는다.
    # 이미 image_to_url이 있는 버전(예: 1.28.2처럼 canvas와 호환되는 버전)에서는
    # 원본을 절대 건드리지 않는다 — 원본을 덮어쓰면 st.pyplot 등 다른 기능이 깨진다.
    try:
...
def render_area_drawer():
    ...
    with st.expander("⚠️ 캔버스에 위성사진 배경이 안 보이나요?"):
        ...
        st.image(base_img, caption="참고용 위성사진 (좌상단이 원점 근처)", **IMAGE_WIDTH_KW)
    ...
    if st.session_state.drawn_areas:
        ...
        edited = st.data_editor(
            df_areas, num_rows="dynamic", key="area_table_editor", **DATA_EDITOR_WIDTH_KW
        )
