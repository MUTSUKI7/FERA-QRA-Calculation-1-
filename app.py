...
with st.sidebar:
    ...
    st.caption(
        ...
        "- F-N Curve: LSIR과 동일 + Area_Coordinates, Occupancy "
        "(선택: Indoor_Vuln_ByBuildingType)"
    )
...
# ------------------------------------------------------------------
def find_area_name_col(df):
    """Area_Coordinates / Occupancy 시트에서 구역명 컬럼을 찾는다. 못 찾으면 첫 컬럼을 사용."""
    c = find_col_by_keywords(df, ["areaname", "area"])
    return c if c is not None else (df.columns[0] if len(df.columns) > 0 else None)


def find_population_col(df, exclude_cols):
    """Occupancy 시트에서 인원수(정수 headcount) 컬럼을 찾는다."""
    c = find_col_by_keywords(
        df, ["population", "headcount", "occupant", "personnel", "peoplecount",
             "numberofpeople", "numpeople", "staff", "worker", "인원"]
    )
    if c is not None:
        return c
    for c in df.columns:
...
def find_occupancy_factor_col(df, exclude_cols):
    """Occupancy 시트에서 재실률(0~1, 해당 인원이 그 자리에 있을 시간비율) 컬럼을 찾는다. 없으면 None."""
    c = find_col_by_keywords(
        df, ["occupancyfactor", "factor", "fraction", "presence", "dutyfactor", "timefraction"]
    )
    return c if c not in exclude_cols else None
...
@st.cache_data(show_spinner=False)
def compute_fn_curve(_excel_bytes, back_frac, cross_frac, default_occ_factor):
    sheets_ = load_excel(_excel_bytes)
    is_coords_df = sheets_["IS_Coordinates"].copy()
    leak_freq_df = sheets_["Leak_Frequencies"].copy()
    phast_df = sheets_["PHAST_Distances"].copy()
    ...
    vuln_df = sheets_["Vulnerability_Criteria"].copy()
    ignition_df = sheets_["Ignition_Probability"].copy()
    area_df = sheets_["Area_Coordinates"].copy()
    occ_df = sheets_["Occupancy"].copy()
    indoor_vuln_df = sheets_.get("Indoor_Vuln_ByBuildingType")
    ...
    # --- Area_Coordinates + Occupancy (인원 분포) ---
    area_name_col = find_area_name_col(area_df)
    ax_col, ay_col = get_xy_cols(area_df)
    bt_col = find_col_by_keywords(area_df, ["buildingtype", "building"])
    if ax_col is None or ay_col is None:
        raise KeyError("Area_Coordinates 시트에 Loc_X_*/Loc_Y_* 컬럼이 없습니다.")

    occ_name_col = find_area_name_col(occ_df)
    pop_col = find_population_col(occ_df, exclude_cols={occ_name_col})
    factor_col = find_occupancy_factor_col(occ_df, exclude_cols={occ_name_col, pop_col})
    if pop_col is None:
        raise KeyError("Occupancy 시트에서 인원수(headcount) 컬럼을 찾지 못했습니다.")

    occ_map = {}
    for _, row in occ_df.iterrows():
        name = str(row[occ_name_col]).strip()
        pop = to_float_or_none(row[pop_col])
        factor = to_float_or_none(row[factor_col]) if factor_col is not None else None
        occ_map[name] = {
            "population": pop if pop is not None else 0.0,
            "factor": factor if factor is not None else float(default_occ_factor),
        }

    areas = []
    for _, row in area_df.iterrows():
        name = str(row[area_name_col]).strip()
        x = to_float_or_none(row[ax_col])
        y = to_float_or_none(row[ay_col])
        if x is None or y is None:
            continue
        occ = occ_map.get(name)
        if occ is None or occ["population"] <= 0:
            continue
        bt = str(row[bt_col]).strip() if bt_col is not None and pd.notna(row[bt_col]) else None
        areas.append({
            "name": name, "x": x, "y": y,
            "population": occ["population"], "factor": occ["factor"], "building_type": bt,
        })
    if not areas:
        raise KeyError(
            "Area_Coordinates와 Occupancy를 연결할 수 있는 유효한 구역이 없습니다 "
            "(구역명 일치 여부, 좌표, 인원수를 확인해주세요)."
        )

    # --- Indoor_Vuln_ByBuildingType (선택, 과압 실내 치사확률) ---
    indoor_vuln = {}
    if indoor_vuln_df is not None and not indoor_vuln_df.empty:
        bt_col2 = next((c for c in indoor_vuln_df.columns if "building" in str(c).lower()),
                        indoor_vuln_df.columns[0])
        col_03 = next((c for c in indoor_vuln_df.columns if "0.3" in str(c)), None)
        col_05 = next((c for c in indoor_vuln_df.columns if "0.5" in str(c)), None)
        for _, row in indoor_vuln_df.iterrows():
            bt_key = str(row[bt_col2]).strip()
            indoor_vuln[bt_key] = {
                "0.3": to_float_or_none(row[col_03]) if col_03 is not None else None,
                "0.5": to_float_or_none(row[col_05]) if col_05 is not None else None,
            }

    def get_exp_vuln(building_type):
        """과압(Overpressure)에 한해 Indoor_Vuln_ByBuildingType이 있으면 그 값을, 없으면
        Vulnerability_Criteria의 실외(Outdoor) 값을 사용한다."""
        if building_type and building_type in indoor_vuln:
            v03 = indoor_vuln[building_type].get("0.3")
            v05 = indoor_vuln[building_type].get("0.5")
            if v03 is not None and v05 is not None:
                return v05, v03
        return v_exp_05, v_exp_03

    # --- 사고 시나리오 누적: (N, F) 이산확률(이항분포) 합성 ---
    max_total_pop = int(round(sum(a["population"] for a in areas)))
    freq_by_N = np.zeros(max_total_pop + 1)
    contrib_by_isid = {}
    ...
    def accumulate_band_event(weight, is_id, center, r_severe, r_mild, bearing, mode,
                               v_severe_default, v_mild_default, use_indoor=False):
        if weight <= 0:
            return
        pmf_items = []
        for a in areas:
            if mode == "lobe":
                sev = lobe_mask(a["x"], a["y"], center, r_severe, bearing, back_frac, cross_frac)
                mld = lobe_mask(a["x"], a["y"], center, r_mild, bearing, back_frac, cross_frac)
            else:
                sev = circle_mask(a["x"], a["y"], center, r_severe)
                mld = circle_mask(a["x"], a["y"], center, r_mild)
            v_sev, v_mld = get_exp_vuln(a["building_type"]) if use_indoor else (v_severe_default, v_mild_default)
            if sev:
                p = a["factor"] * v_sev
            elif mld:
                p = a["factor"] * v_mld
            ...
            p = min(max(p, 0.0), 1.0)
            if p > 0:
                pmf_items.append((a["population"], p))
        _combine_and_add(weight, is_id, pmf_items)

    def accumulate_single_event(weight, is_id, center, r, bearing, mode, v_default):
        if weight <= 0 or r is None or r <= 0:
            return
        pmf_items = []
        for a in areas:
            hit = lobe_mask(a["x"], a["y"], center, r, bearing, back_frac, cross_frac) if mode == "lobe" \
                else circle_mask(a["x"], a["y"], center, r)
            p = min(max(a["factor"] * v_default, 0.0), 1.0) if hit else 0.0
            if p > 0:
                pmf_items.append((a["population"], p))
        _combine_and_add(weight, is_id, pmf_items)
            ...
            for wr in wind_rows:
                ...
                if cls["has_jet"]:
                    ...
                    accumulate_band_event(weight_vce, is_id, center, d["Exp_0.5"], d["Exp_0.3"], bearing,
                                           "circle", v_exp_05, v_exp_03, use_indoor=True)

    # --- 누적빈도 F(N 이상) 계산 (N은 항상 정수) ---
    n_all = np.arange(0, max_total_pop + 1)
    ...
    if len(nonzero_idx) == 0:
        return np.array([]), np.array([]), pd.DataFrame(), areas
    last = nonzero_idx[-1]
    ...
    return n_values, f_cum, event_summary, areas


def render_fn_curve():
    required = ["IS_Coordinates", "Leak_Frequencies", "PHAST_Distances",
                "Wind rose(Meteorology)", "Vulnerability_Criteria", "Ignition_Probability",
                "Area_Coordinates", "Occupancy"]
    miss = missing_sheets(required)
    if miss:
        st.warning(f"이 분석에 필요한 시트가 없습니다: {miss}")
        return
    ...
    st.caption(
        "방법론(이산확률/이항분포 방식): (IS_ID, 누출구경, 기상조건, 풍향, 사고분기) 조합별로 "
        "빈도(=누출빈도 x 풍향확률 x 착화확률)를 산정하고, 그 영향범위 안에 있는 각 Area의 인원수(정수)에 "
        "치사확률을 성공확률로 하는 이항분포를 적용해 정수 N(사망자 수)의 분포를 구합니다. "
        "구역 간 독립을 가정해 합성곱(convolution)으로 결합하고, 모든 시나리오를 합산해 "
        "F(N명 이상 사망 누적빈도)를 계산하므로 N은 항상 정수입니다."
    )
    default_occ_factor = st.slider(
        "기본 재실률 가정 (Occupancy 시트에 재실률 컬럼이 없을 때 전 구역에 적용)",
        0.0, 1.0, 1.0, 0.05, key="fn_default_occ_factor"
    )
    show_markers = st.checkbox("계산 지점에 마커 표시", value=False, key="fn_show_markers")

    with st.spinner("F-N Curve 계산 중..."):
        try:
            n_values, f_cum, event_summary, areas = compute_fn_curve(
                excel_bytes, back_frac, cross_frac, default_occ_factor
            )
        except Exception as e:
            st.error(f"F-N Curve 계산 중 오류가 발생했습니다 (엑셀 컬럼명이 템플릿과 다를 수 있습니다): {e}")
            return

    st.caption(f"인원 데이터가 연결된 구역 수: {len(areas)}개, 총 인원: {sum(a['population'] for a in areas):.0f}명")

    if len(n_values) == 0:
        st.warning("계산된 인명피해(N≥1) 이벤트가 없습니다 — Occupancy/Area_Coordinates 데이터를 확인해주세요.")
        return
