    ...
    try:
        ...
        if not hasattr(_st_image_mod, "image_to_url"):
            ...
            def _image_to_url_patch(image, width=None, clamp=False, channels="RGB",
                                     output_format="auto", image_id=None):
                buf = _BytesIO()
...
def render_area_drawer():
    ...
    st.caption(
        "1) 아래 '그리기 모드'에서 사각형을 그리고 '목록에 추가'를 누르세요. "
        "2) 회전시키려면 모드를 '이동/크기조정/회전'으로 바꾼 뒤 사각형을 클릭해 선택하고, "
        "도형 위에 나타나는 회전 손잡이(작은 원)를 마우스로 돌리세요. "
        "3) 표에서 Area 이름을 Occupancy 시트의 구역(컬럼)명과 맞게 고쳐주세요 "
