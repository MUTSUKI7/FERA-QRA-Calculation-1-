...
def render_fn_curve():
    ...
    # 마지막 계단(최대 N 지점)이 그래프 오른쪽 끝(x_hi)에 딱 붙어서 잘려 보이지 않도록,
    # x축을 최대 N보다 여유 있게(15%) 확장하고, 그 여유 구간까지 마지막 F값을 수평으로 이어 그린다.
    # (실제 데이터는 n_values/f_cum까지이며, 이 연장선은 시각적 여백일 뿐 새로운 시나리오가 아님)
    x_lo = 1
    x_hi = max(n_values[-1] * 1.15, 2)
    n_plot = np.append(n_values, x_hi)
    f_plot = np.append(f_cum, f_cum[-1])
    ax.step(n_plot, f_plot, where="post", color="tab:blue", linewidth=2)
    if show_markers:
        # 마커는 실제 계산된 지점(n_values, f_cum)에만 표시하고, 오른쪽 여백 연장점에는 찍지 않는다.
