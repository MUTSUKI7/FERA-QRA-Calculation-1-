...
with st.sidebar:
    ...
    st.caption(
        "아래 두 값은 업로드한 위성사진이 실제로 담고 있는 가로/세로 거리(m)입니다. "
        "Google Earth 축척바 등으로 직접 실측한 값을 입력하세요 — 이 값이 부정확하면 "
        "모든 IS 위치·위험반경·격자·F-N Curve 결과가 실제 지형과 어긋나게 표시됩니다."
    )
    col_ex, col_ey = st.columns(2)
    with col_ex:
        grid_extent_x_m = st.number_input(
            "위성사진 실제 가로 길이 (m)", min_value=1.0, value=1000.0, step=1.0
        )
    with col_ey:
        grid_extent_y_m = st.number_input(
            "위성사진 실제 세로 길이 (m)", min_value=1.0, value=1000.0, step=1.0
        )
    cell_size_m = st.number_input(
        "격자 한 칸 크기 (m)",
        min_value=0.1, value=50.0, step=1.0,
    )
    grid_cols = grid_extent_x_m / cell_size_m
    grid_rows = grid_extent_y_m / cell_size_m
    show_grid_lines = st.checkbox("이미지 위에 격자선 그리기", value=True)

    with st.expander("고급 설정 — IS_Coordinates 좌표 기준 범위"):
        st.caption(
            "IS_Coordinates 시트의 Loc_X/Loc_Y 값은 엑셀 작성 당시 가정했던 격자 범위(보통 1000m x 1000m) "
            "기준으로 입력되어 있을 수 있습니다. 위 '위성사진 실제 가로/세로 길이'를 실측값으로 바꿔도 "
            "누출지점이 이미지 위에서 원래 표시되던 위치(비율)를 그대로 유지하도록, 엑셀 좌표가 원래 "
            "어떤 범위를 기준으로 작성됐는지를 여기에 입력합니다. (엑셀을 다시 만들지 않는 한 보통 "
            "그대로 두면 됩니다.)"
        )
        col_rx, col_ry = st.columns(2)
        with col_rx:
            coord_ref_extent_x_m = st.number_input(
                "IS 좌표 작성 시 가정한 가로 범위 (m)", min_value=1.0, value=1000.0, step=1.0
            )
        with col_ry:
            coord_ref_extent_y_m = st.number_input(
                "IS 좌표 작성 시 가정한 세로 범위 (m)", min_value=1.0, value=1000.0, step=1.0
            )
    ...
    with st.expander("고급 설정 — 로브(lobe) 형태 가정치 (열복사·LSIR에 적용)"):
        ...
        st.caption(
            "위 슬라이더는 두 물리 모델이 꺼져 있거나 필요 데이터가 없는 시나리오에 적용되는 "
            "고정값입니다."
        )
        st.caption(
            "**Pool fire**: AGA/Thomas(1963)의 화염 기울기(flame tilt, θ) 상관식으로 '모양'을, "
            "θ=0(원형)↔θ=90(=Pool_Diameter_m/2 ÷ Pool_12.5, 화염 자신의 물리적 크기 하한)의 두 "
            "물리적 경계값을 cos(θ)로 보간해 IS_ID·기상조건별 BACK_FRAC=CROSS_FRAC을 자동 계산 "
            "(PHAST_Distances의 Pool_Diameter_m·Pool_BurnRate_kgs·Pool_12.5, Wind rose의 "
            "Wind_Speed_(m/s) 필요)."
        )
        use_physics_pool = st.checkbox(
            "Pool Fire에 유체역학(AGA/Thomas) + 기하학적 경계값 결합 모델 적용", value=True
        )
        st.caption(
            "**Jet fire**: 검증된 화염 기울기 상관식을 확보하지 못해(θ 보간 없음), Pool fire와 "
            "동일한 원리의 기하학적 하한만 적용 — BACK_FRAC = CROSS_FRAC = Jet_FlameLength_m ÷ "
            "Jet_12.5 ('화염 자신의 길이보다 위험거리가 좁아질 이유가 없다'는 논거, DNV GL Table 1의 "
            "SEP=250 kW/m² 고정 계수는 우리 데이터와 최대 89% 어긋나 검증 실패해 채택하지 않음)."
        )
        use_physics_jet = st.checkbox(
            "Jet Fire에 화염 길이 기반 기하학적 로브 형태 적용", value=True
        )
...
def load_excel(excel_bytes):
    # 인자 이름이 밑줄(_)로 시작하면 Streamlit이 캐시 키 계산에서 그 값을 무시한다
    # (해시 불가능한 인자를 위한 규칙). 여기서는 실제 파일 내용(bytes, 해시 가능)이므로
    # 밑줄을 붙이지 않아야 "새 엑셀 파일을 올려도 캐시가 갱신되지 않는" 문제가 생기지 않는다.
...
def rescale_is_coords(df, x_col, y_col, ref_extent_x_m, ref_extent_y_m, real_extent_x_m, real_extent_y_m):
    """IS_Coordinates(Loc_X/Loc_Y)는 엑셀 작성 당시 가정한 격자 범위(ref_extent, 보통 1000x1000m)
    기준의 좌표다. 위성사진의 실제 가로/세로 길이(real_extent)가 그 가정과 다르더라도, 각 지점이
    이미지 안에서 차지하던 상대 위치(비율)는 그대로 유지한 채 실제 미터 단위로 환산해준다.
    즉 real_x = excel_x * (real_extent_x_m / ref_extent_x_m) 로, 위성사진 실제 길이를 바꿔도
    누출지점이 화면상에서 이동하지 않도록 한다."""
    df = df.copy()
    if ref_extent_x_m and ref_extent_x_m > 0:
        df[x_col] = df[x_col] * (real_extent_x_m / ref_extent_x_m)
    if ref_extent_y_m and ref_extent_y_m > 0:
        df[y_col] = df[y_col] * (real_extent_y_m / ref_extent_y_m)
    return df
...
def m_to_px_factory(img_size, grid_extent_x_m, grid_extent_y_m):
    """이미지 픽셀 크기와 실제 가로/세로 거리(m)로부터 좌표 변환 함수를 만든다.
    가로/세로 스케일(px_per_m_x, px_per_m_y)을 따로 두어, 위성사진의 가로/세로 실측
    거리가 이미지의 가로세로 픽셀 비율과 정확히 같지 않아도(즉 정사각형이 아니어도)
    각 축의 실제 미터 단위 거리가 올바르게 변환되도록 한다."""
    W, H = img_size
    px_per_m_x = W / grid_extent_x_m
    px_per_m_y = H / grid_extent_y_m

    def m_to_px(x_m, y_m):
        return x_m * px_per_m_x, H - y_m * px_per_m_y

    return m_to_px, px_per_m_x, px_per_m_y


def draw_grid_lines(draw, img_size, cell_size_m, px_per_m_x, px_per_m_y, color=(255, 255, 255, 110), width=1):
    """이미지 위에 cell_size_m 간격의 격자선을 그린다 (가로/세로 스케일이 다를 수 있음)."""
    W, H = img_size
    cell_px_x = cell_size_m * px_per_m_x
    cell_px_y = cell_size_m * px_per_m_y
    if cell_px_x >= 2:
        x = 0.0
        while x <= W:
...
# ------------------------------------------------------------------
# Pool Fire의 BACK_FRAC/CROSS_FRAC — AGA/Thomas(1963) 화염 기울기(theta) + 물리적 경계값 결합
#
# theta(화염 기울기 각)는 AGA/Thomas의 무차원 풍속(u*) 상관식으로 계산한다 (문헌 검증됨):
#     u* = U / (g * m'' * D / rho_air)^(1/3),  cos(theta) = 1 (u*<=1) / 1/u* (u*>1)
# theta는 "형태(모양)"를 결정하고, PHAST가 계산한 R(위험거리)은 "크기"를 결정한다 — 서로 다른
# 정보라 이중계산이 아니다: PHAST_Distances에는 애초에 풍하측 거리 R 하나만 있고 형태 정보 자체가
# 없으므로, theta로 그 형태를 보충하는 것은 R과 무관하게 필요한 별도의 역할이다.
#
# theta -> BACK_FRAC/CROSS_FRAC 변환의 두 끝점을 임의로 정하지 않고 물리적으로 도출한다:
#   - theta=0  (화염이 안 누움)  -> frac=1.0  (방향성이 없다면 PHAST의 R 자체가 '사방으로 R만큼
#     위험하다'는 뜻이 되어야 하므로, 대칭인 원(반지름 R)이 정답 -- 임의값이 아니라 R의 정의에서 나옴)
#   - theta=90 (화염이 완전히 누움) -> frac=(D_pool/2)/R_hazard  (풍상측·좌우측 모두, 화염 자신의
#     물리적 크기보다 위험거리가 작아질 이유가 없다는 기하학적 하한)
# 그 사이는 cos(theta)로 보간한다: 기울어진 물체의 그림자 길이가 기울기의 코사인에 비례한다는
# 자연스러운 삼각함수적 형태이지만, 이 특정 용도(ellipse 비율)에 대해 논문으로 검증된 형태는 아니므로
# 여전히 근사임을 명시한다. 남은 임의성은 이 보간 곡선의 '모양'뿐이고(전에는 4개 상수+2개 함수
# 형태가 임의였음), 두 끝점 자체는 더 이상 임의가 아니다.
#
# CROSS_FRAC(좌우 폭)에는 검증된 좌우폭 전용 상관식이 없어, BACK_FRAC과 동일한 식을 그대로 쓴다
# (보수적 단순화임을 명시).
# (Jet fire는 분출속도 데이터가 없어 이 모델 자체를 적용할 수 없고, 여전히 고정 슬라이더 값을 쓴다.)
# ------------------------------------------------------------------
def pool_fire_tilt_deg(wind_speed_ms, burn_rate_kgs, pool_diameter_m,
                        rho_air=1.2, g=9.81):
    """AGA/Thomas(1963) cosine 모델로 화염 기울기 각(도, 수직 기준)을 계산한다.
    u* = U / (g * m'' * D / rho_air)^(1/3), cos(theta) = 1(u*<=1) / 1/u*(u*>1).
    필요 데이터가 없거나 물리적으로 무효하면 None을 반환한다."""
    if wind_speed_ms is None or burn_rate_kgs is None or pool_diameter_m is None:
        return None
    if pd.isna(wind_speed_ms) or pd.isna(burn_rate_kgs) or pd.isna(pool_diameter_m):
        return None
    if burn_rate_kgs <= 0 or pool_diameter_m <= 0:
        return None
    u_c = (g * burn_rate_kgs * pool_diameter_m / rho_air) ** (1.0 / 3.0)
    if u_c <= 0:
        return None
    u_star = wind_speed_ms / u_c
    cos_theta = 1.0 if u_star <= 1.0 else 1.0 / u_star
    cos_theta = max(0.0, min(1.0, cos_theta))
    return math.degrees(math.acos(cos_theta))


def get_wind_speed_by_class(wind_rose_df):
    """Wind rose(Meteorology) 시트에서 Weather_Class별 대표 풍속(m/s)을 뽑아낸다."""
    speed_col = next(
        (c for c in wind_rose_df.columns if "windspeed" in str(c).lower().replace(" ", "").replace("_", "")),
        None,
    )
    if speed_col is None or "Weather_Class" not in wind_rose_df.columns:
        return {}
    df = wind_rose_df.copy()
    df[speed_col] = pd.to_numeric(df[speed_col], errors="coerce")
    return df.groupby("Weather_Class")[speed_col].mean().to_dict()


def resolve_pool_fracs(use_physics, wind_speed_ms, burn_rate_kgs, pool_diameter_m, hazard_radius_m,
                        fallback_back, fallback_cross, numeric_floor=0.02):
    """theta(AGA/Thomas)와 물리적으로 도출한 두 끝점(theta=0->1.0, theta=90->(D/2)/R)을 결합해
    BACK_FRAC=CROSS_FRAC을 계산한다. 필요 데이터가 부족하면 가능한 만큼만 물리적으로 계산하고
    (풀 직경·위험거리만 있고 풍속·연소율이 없으면 기하학적 하한만이라도 사용), 그마저도 없으면
    사이드바 슬라이더 값을 그대로 반환한다."""
    if not use_physics:
        return fallback_back, fallback_cross
    if pool_diameter_m is None or hazard_radius_m is None or hazard_radius_m <= 0:
        return fallback_back, fallback_cross
    if pd.isna(pool_diameter_m) or pd.isna(hazard_radius_m):
        return fallback_back, fallback_cross

    frac_min = max(numeric_floor, min(1.0, (pool_diameter_m / 2.0) / hazard_radius_m))
    theta_deg = pool_fire_tilt_deg(wind_speed_ms, burn_rate_kgs, pool_diameter_m)
    if theta_deg is None:
        # 풍속/연소율 데이터가 없어 theta를 못 구하면, 물리적 하한만이라도 사용 (완전 fallback보다 나음)
        return frac_min, frac_min
    theta = math.radians(min(max(theta_deg, 0.0), 90.0))
    frac = frac_min + (1.0 - frac_min) * math.cos(theta)
    return frac, frac


# ------------------------------------------------------------------
# Jet Fire의 BACK_FRAC/CROSS_FRAC — Pool fire와 동일한 논리의 순수 기하학적 하한
#
# 최초 검토했던 DNV GL Hazards 26 논문(Table 1, 화염 길이->타원 등고선 변환 계수 a/b)은 실제
# 데이터로 검증해보니 SEP=250 kW/m^2 고정 가정 때문에 우리 데이터(SEP가 시나리오마다 다름)와
# 최대 89% 어긋나 그대로 쓸 수 없었다 (검증 기록: 2mm 홀에서 예측/실제=1.89배).
#
# 대신 Pool fire에 썼던 것과 동일한 원리를 적용한다: 화염 자신의 물리적 길이(L_flame)를 넘어서까지
# 위험거리가 좁아질 이유가 없다는 기하학적 하한.
#     JET_FRAC = L_flame / R_hazard   (Pool의 (D_pool/2)/R와 같은 역할 -- Jet은 점원에서 뻗어나가는
#     길이라 반경 개념인 L_flame을 그대로 쓰고, Pool처럼 별도로 2로 나누지 않는다: Pool의 D_pool은
#     '지름'이라 반지름을 쓰려면 절반이 필요했지만, L_flame은 이미 방출점 기준의 '거리'이기 때문)
#
# Pool의 v3(AGA/Thomas theta로 두 끝점을 보간하는 결합모델)과 달리, Jet fire를 위한 검증된 화염
# 기울기(theta) 상관식은 확보하지 못했으므로 (Kalghatgi(1983) 상관식은 각도 정의(수직/수평 기준)를
# 원 논문에서 확인하지 못해 부호·기준을 신뢰할 수 없다고 이전에 밝힌 바 있음), theta 보간 없이
# 이 기하학적 하한 값 하나만 사용한다 -- Pool의 v2(순수 기하학적 버전)와 동일한 수준의 근거다.
# ------------------------------------------------------------------
def resolve_jet_fracs(use_physics, flame_length_m, hazard_radius_m,
                       fallback_back, fallback_cross, numeric_floor=0.02):
    """물리 모델 스위치가 켜져 있고 필요 데이터(화염 길이, PHAST 위험거리)가 모두 있으면
    BACK_FRAC = CROSS_FRAC = L_flame / R_hazard 를 계산해 반환한다. 데이터가 없거나 스위치가
    꺼져 있으면 사이드바 슬라이더 값을 그대로 반환한다."""
    if not use_physics:
        return fallback_back, fallback_cross
    if flame_length_m is None or hazard_radius_m is None or hazard_radius_m <= 0:
        return fallback_back, fallback_cross
    if pd.isna(flame_length_m) or pd.isna(hazard_radius_m):
        return fallback_back, fallback_cross

    ratio = max(numeric_floor, min(1.0, flame_length_m / hazard_radius_m))
    return ratio, ratio
...
def render_explosion():
    ...
    is_coords = rescale_is_coords(
        is_coords, x_col, y_col,
        coord_ref_extent_x_m, coord_ref_extent_y_m, grid_extent_x_m, grid_extent_y_m,
    )
    ...
    img = base_img.copy()
    m_to_px, px_per_m_x, px_per_m_y = m_to_px_factory(img.size, grid_extent_x_m, grid_extent_y_m)
    points = {
    ...
    def union_outline_contours(level_col):
        ...
        for _, row in envelope.iterrows():
            ...
            if pd.isna(r_m) or is_id not in points:
                continue
            cx, cy = points[is_id]
            # 실제 위험거리(r_m)는 등방(원형)이지만, 가로/세로 스케일(px_per_m_x, px_per_m_y)이
            # 다를 수 있어 픽셀상으로는 타원으로 그려야 실제 미터 단위 원과 일치한다.
            rx_px = r_m * px_per_m_x
            ry_px = r_m * px_per_m_y
            if rx_px <= 0 or ry_px <= 0:
                continue
            cv2.ellipse(
                mask, (int(round(cx)), int(round(cy))), (int(round(rx_px)), int(round(ry_px))),
    ...
    if show_grid_lines:
        draw_grid_lines(draw, img.size, cell_size_m, px_per_m_x, px_per_m_y)
    ...
    legend_draw.text(
        (lx + 8, note_y),
        f"Grid: {cell_size_m:.1f}m/cell ({grid_extent_x_m:.0f}m x {grid_extent_y_m:.0f}m, "
        f"{round(grid_cols)}x{round(grid_rows)} cells).",
...
def render_thermal():
    ...
    for c in ["Jet_12.5", "Jet_37.5", "Pool_12.5", "Pool_37.5", "Pool_Diameter_m", "Pool_BurnRate_kgs",
              "Jet_FlameLength_m"]:
        if c not in phast.columns:
            phast[c] = np.nan
        else:
    ...
    by_weather = phast.groupby(["IS_ID", "Weather_Class"])[
        ["Jet_12.5", "Jet_37.5", "Pool_12.5", "Pool_37.5",
         "Pool_Diameter_m", "Pool_BurnRate_kgs", "Jet_FlameLength_m"]].max().reset_index()
    wind_speed_by_class = get_wind_speed_by_class(wind_rose)
        ...
        for _, wr_row in wind_rose.iterrows():
            ...
            for _, row in matches.iterrows():
                ...
                if pd.isna(R) or R <= 0:
                    continue
                if ftype == "Pool fire":
                    bf, cf = resolve_pool_fracs(
                        use_physics_pool,
                        wind_speed_by_class.get(wc),
                        row.get("Pool_BurnRate_kgs"),
                        row.get("Pool_Diameter_m"),
                        row.get("Pool_12.5"),
                        back_frac, cross_frac,
                    )
                elif ftype == "Jet fire":
                    bf, cf = resolve_jet_fracs(
                        use_physics_jet,
                        row.get("Jet_FlameLength_m"),
                        row.get("Jet_12.5"),
                        back_frac, cross_frac,
                    )
                else:
                    bf, cf = back_frac, cross_frac
                poly = lobe_polygon_px(points_m[is_id], R, bearing, bf, cf, m_to_px)
    ...
    legend_draw.text((lx + 8, note_y), "Directional lobes per Wind rose (ASSUMED shape, not raw PHAST output):",
                      fill=(230, 230, 230, 255), font=FONT_SMALL)
    pool_note = "Pool fire=AGA/Thomas theta + geometric bound model" if use_physics_pool else "Pool fire=fixed slider value"
    legend_draw.text((lx + 8, note_y + 15),
                      f"  Jet fire: upwind={back_frac:.0%} R, crosswind={cross_frac:.0%} R (fixed).  {pool_note}.",
                      fill=(230, 230, 230, 255), font=FONT_SMALL)
    legend_draw.text(
        (lx + 8, note_y + 30),
        f"Grid: {cell_size_m:.1f}m/cell ({grid_extent_x_m:.0f}m x {grid_extent_y_m:.0f}m, "
...
@st.cache_data(show_spinner=False)
def compute_lsir_field(
    excel_bytes, grid_extent_x_m, grid_extent_y_m,
    coord_ref_extent_x_m, coord_ref_extent_y_m,
    grid_res_m, back_frac, cross_frac, use_physics_pool=True, use_physics_jet=True,
):
    # 버그 수정: 이전 버전은 use_physics_jet을 파라미터로 받지 않고 전역 변수를 직접 참조했다.
    # st.cache_data는 '함수에 전달된 파라미터 값'만으로 캐시를 구분하므로, Jet Fire 물리 모델
    # 체크박스를 켜고 꺼도 캐시 키가 바뀌지 않아 이전 결과가 그대로 재사용되는 문제가 있었다.
    # use_physics_jet을 명시적 파라미터로 추가해 체크박스 상태가 캐시 키에 반영되도록 한다.
    sheets_ = load_excel(excel_bytes)
    ...
    is_coords_df = rescale_is_coords(
        is_coords_df, x_col, y_col,
        coord_ref_extent_x_m, coord_ref_extent_y_m, grid_extent_x_m, grid_extent_y_m,
    )
    is_coords = {
    ...
    pool_physics_cols = [c for c in ["Pool_Diameter_m", "Pool_BurnRate_kgs"] if c in phast_df.columns]
    jet_physics_cols = [c for c in ["Jet_FlameLength_m"] if c in phast_df.columns]
    ...
    hole_col_phast = find_hole_size_col(
        ...
        | set(pool_physics_cols) | set(jet_physics_cols)
    )
    ...
    for _, row in phast_df.iterrows():
        ...
        phast[key] = {c: to_float_or_none(row[c]) for c in thermal_level_cols + pool_physics_cols + jet_physics_cols}
        for c in ["Jet_12.5", "Jet_37.5", "Pool_12.5", "Pool_37.5", "Flash_LFL", "Exp_0.3", "Exp_0.5",
                  "Pool_Diameter_m", "Pool_BurnRate_kgs", "Jet_FlameLength_m"]:
    ...
    wind_speed_by_class = get_wind_speed_by_class(wind_rose_df)
    ...
    n_x = int(grid_extent_x_m / grid_res_m) + 1
    n_y = int(grid_extent_y_m / grid_res_m) + 1
    xs = np.linspace(0, grid_extent_x_m, n_x)
    ys = np.linspace(0, grid_extent_y_m, n_y)
        ...
        for _, hs in hole_sizes:
            ...
            for wr in wind_rows:
                ...
                if cls["has_pool"]:
                    ...
                    bf_pool, cf_pool = resolve_pool_fracs(
                        use_physics_pool, wind_speed_by_class.get(wc),
                        d.get("Pool_BurnRate_kgs"), d.get("Pool_Diameter_m"), d.get("Pool_12.5"),
                        back_frac, cross_frac,
                    )
                    ms = lobe_mask(X, Y, center, d["Pool_37.5"], bearing, bf_pool, cf_pool)
                    mm = lobe_mask(X, Y, center, d["Pool_12.5"], bearing, bf_pool, cf_pool)
                    add_band_contribution(risk_field, ms, mm, v_thermal_375, v_thermal_125, weight)
                if cls["has_jet"]:
                    weight_jet = freq * wind_p * ign["P_jet"]
                    bf_jet, cf_jet = resolve_jet_fracs(
                        use_physics_jet, d.get("Jet_FlameLength_m"), d.get("Jet_12.5"),
                        back_frac, cross_frac,
                    )
                    ms = lobe_mask(X, Y, center, d["Jet_37.5"], bearing, bf_jet, cf_jet)
                    mm = lobe_mask(X, Y, center, d["Jet_12.5"], bearing, bf_jet, cf_jet)
...
def render_lsir():
    ...
    with st.spinner("LSIR 계산 중..."):
        try:
            X, Y, risk_field, is_coords = compute_lsir_field(
                excel_bytes, grid_extent_x_m, grid_extent_y_m,
                coord_ref_extent_x_m, coord_ref_extent_y_m,
                grid_res_m, back_frac, cross_frac, use_physics_pool, use_physics_jet,
            )
        except Exception as e:
    ...
    ax.imshow(np.asarray(base_img), extent=[0, grid_extent_x_m, 0, grid_extent_y_m], origin="upper")
    ax.set_aspect("equal")
    if show_grid_lines:
        xticks = np.arange(0, grid_extent_x_m + 1e-6, cell_size_m)
        yticks = np.arange(0, grid_extent_y_m + 1e-6, cell_size_m)
    ...
    ax.set_xlim(0, grid_extent_x_m)
    ax.set_ylim(0, grid_extent_y_m)
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
...
def _area_corners_px(cx, cy, w, h, angle_deg, px_per_m_x, px_per_m_y, canvas_w, canvas_h):
    """m 좌표계의 (중심, 가로세로, 회전각)을 가진 사각형의 4개 꼭짓점을
    캔버스 픽셀 좌표로 변환한다 (가로/세로 스케일이 다를 수 있음)."""
    hw, hh = w / 2.0, h / 2.0
    ...
    for lx, ly in local:
        ...
        wy = cy + lx * sin_t + ly * cos_t
        px = wx * px_per_m_x
        py = canvas_h - wy * px_per_m_y
        pts_px.append((px, py))
    return pts_px


def draw_areas_overlay(bg_img, areas_records, canvas_w, canvas_h, grid_extent_x_m, grid_extent_y_m, pending_corner=None):
    """지금까지 등록된 Area 사각형(+이름)과, 클릭 대기 중인 첫 모서리 표시를
    위성사진 위에 그려서 보여준다."""
    img = bg_img.convert("RGBA").copy()
    overlay = Image.new("RGBA", img.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    px_per_m_x = canvas_w / grid_extent_x_m
    px_per_m_y = canvas_h / grid_extent_y_m
    try:
    ...
    for a in areas_records:
        ...
        if w <= 0 or h <= 0:
            continue
        pts = _area_corners_px(cx, cy, w, h, angle, px_per_m_x, px_per_m_y, canvas_w, canvas_h)
...
def detect_rectangles_from_annotated_image(
    ...
    (m_to_px_factory, _area_corners_px 등에서 쓰는 것과 같은 px_per_m_x = W/extent_x_m,
    px_per_m_y = H/extent_y_m 규약).
    """
    pil_img = Image.open(io.BytesIO(image_bytes)).convert("RGB")
    img_rgb = np.array(pil_img)
    img_bgr = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR)
    h_px, w_px = img_bgr.shape[:2]
    px_per_m_x = w_px / grid_extent_x_m
    px_per_m_y = h_px / grid_extent_y_m
    ...
    for cnt in contours:
        if cv2.contourArea(cnt) < min_area_px:
            continue
        rect_px = cv2.minAreaRect(cnt)
        box_px = cv2.boxPoints(rect_px)  # 4 corners in pixel space (order: consistent per cv2 version)

        # 가로/세로 스케일(px_per_m_x, px_per_m_y)이 다를 수 있으므로, 픽셀 좌표계의
        # 회전된 사각형을 그대로 각도/변길이로 나누지 않고, 각 꼭짓점을 먼저
        # (이 앱의 좌표계: 원점 좌하단, y위쪽 양수) 실제 미터 좌표로 변환한 뒤
        # 그 미터 좌표 위에서 중심/가로세로/회전각을 다시 계산한다.
        box_m = np.array([
            [px / px_per_m_x, (h_px - py) / px_per_m_y] for (px, py) in box_px
        ])
        cx_m, cy_m = box_m[:, 0].mean(), box_m[:, 1].mean()

        edge1 = box_m[1] - box_m[0]
        edge2 = box_m[2] - box_m[1]
        len1, len2 = np.linalg.norm(edge1), np.linalg.norm(edge2)
        if len1 >= len2:
            long_edge, w_m, h_m = edge1, len1, len2
        else:
            long_edge, w_m, h_m = edge2, len2, len1
        angle = math.degrees(math.atan2(long_edge[1], long_edge[0]))
        # atan2 결과는 -180~180 범위이며 180도 차이는 같은 변이므로 -90~90으로 정규화
        angle = ((angle + 90) % 180) - 90

        if flip_rotation_sign:
            # 방향이 반대로 보이면(예: cv2 버전별 꼭짓점 순서 차이) 체크박스로 뒤집기
            angle = -angle

        records.append({
            "Area_Name": "",  # 아래 표에서 채움
            "Loc_X_center": round(float(cx_m), 1),
            "Loc_Y_center": round(float(cy_m), 1),
            "Width_m": round(float(w_m), 1),
            "Height_m": round(float(h_m), 1),
...
def render_area_drawer():
    ...
    if rect_img_file is not None:
        ...
        try:
            detected, mask_preview = detect_rectangles_from_annotated_image(
                rect_img_file.getvalue(), grid_extent_x_m, grid_extent_y_m,
    ...
    if not HAS_CLICK:
        ...
        # 캔버스는 정사각형으로 고정하지 않고, 위성사진의 픽셀 가로세로 비율 그대로
        # 리사이즈해서 화면이 찌그러져 보이지 않게 한다. 실제 미터 환산은
        # grid_extent_x_m / grid_extent_y_m 을 각각 가로/세로에 따로 적용한다.
        canvas_w = 700
        canvas_h = max(1, round(canvas_w * base_img.height / base_img.width))
        bg_img_resized = base_img.resize((canvas_w, canvas_h))
        px_per_m_x_canvas = canvas_w / grid_extent_x_m
        px_per_m_y_canvas = canvas_h / grid_extent_y_m
        ...
        display_img = draw_areas_overlay(
            bg_img_resized, st.session_state.drawn_areas, canvas_w, canvas_h,
            grid_extent_x_m, grid_extent_y_m,
        ...
        if click_value is not None and click_value != st.session_state.area_click_last:
            ...
            cx_click_m = px / px_per_m_x_canvas
            cy_click_m = (canvas_h - py) / px_per_m_y_canvas
...
@st.cache_data(show_spinner=False)
def compute_fn_curve(
    excel_bytes, back_frac, cross_frac, areas_json,
    grid_extent_x_m, grid_extent_y_m,
    coord_ref_extent_x_m, coord_ref_extent_y_m,
    use_physics_pool=True, use_physics_jet=True,
):
    # 버그 수정: compute_lsir_field와 동일한 문제 — use_physics_jet이 파라미터가 아니라 전역 변수를
    # 직접 참조하고 있어, Jet Fire 물리 모델 체크박스를 켜고 꺼도 st.cache_data의 캐시 키가 바뀌지
    # 않아 F-N Curve 결과가 갱신되지 않는 문제가 있었다. 명시적 파라미터로 승격해 수정한다.
    ...
    def expected_band_event(weight, is_id, center, r_severe, r_mild, bearing, mode, hazard_type,
                             level_severe, level_mild, bf=None, cf=None):
        if weight <= 0:
            return
        bf = back_frac if bf is None else bf
        cf = cross_frac if cf is None else cf
        n_event = 0.0
        for a in matched_areas:
            ...
            frac_sev, frac_mld = rect_band_fractions(
                ...
                center, r_severe, r_mild, bearing, mode, bf, cf
            )
            ...
            for wr in wind_rows:
                ...
                if cls["has_pool"]:
                    ...
                    expected_band_event(weight, is_id, center, d["Pool_37.5"], d["Pool_12.5"], bearing,
                                         "lobe", "Thermal (Jet/Pool)", "37.5", "12.5", bf=bf_pool, cf=cf_pool)
                if cls["has_jet"]:
                    weight_jet = freq * wind_p * ign["P_jet"]
                    bf_jet, cf_jet = resolve_jet_fracs(
                        use_physics_jet, d.get("Jet_FlameLength_m"), d.get("Jet_12.5"),
                    ...
                    expected_band_event(weight_jet, is_id, center, d["Jet_37.5"], d["Jet_12.5"], bearing,
                                         "lobe", "Thermal (Jet/Pool)", "37.5", "12.5", bf=bf_jet, cf=cf_jet)
    ...
    if not events:
        return np.array([]), np.array([]), pd.DataFrame(), matched_areas, 0.0, sorted({r["area_name"] for r in occ_records})
    ...
    occ_area_names_all = sorted({r["area_name"] for r in occ_records})
    return n_values, f_cum, event_summary, matched_areas, pll_total, occ_area_names_all
...
def render_fn_curve():
    ...
    with st.spinner("F-N Curve 계산 중..."):
        try:
            ...
            n_values_all, f_cum_all, event_summary, matched_areas, pll_total, occ_area_names_all = compute_fn_curve(
                excel_bytes, back_frac, cross_frac, areas_json,
                grid_extent_x_m, grid_extent_y_m,
                coord_ref_extent_x_m, coord_ref_extent_y_m,
                use_physics_pool, use_physics_jet,
            )
        except Exception as e:
    ...
    if n_matched < len(matched_areas):
        ...
        # 화면상으로는 똑같아 보여도 보이지 않는 공백/오타 때문에 매칭이 안 되는 경우가 많아,
        # Occupancy 시트에 있는 이름 중 가장 비슷한 후보를 같이 보여준다 (원인 파악용).
        import difflib
        for name in unmatched:
            close = difflib.get_close_matches(name, occ_area_names_all, n=3, cutoff=0.4)
            if close:
                st.caption(f"  · '{name}' ↔ Occupancy 시트에서 가장 비슷한 이름: {close} "
                           "(철자는 같아 보여도 안 보이는 공백/오타가 있을 수 있습니다 — 표에서 해당 셀을 "
                           "완전히 지우고 다시 입력해보세요)")
    ...
    # x축은 최대 N보다 여유 있게(15%) 확장해 오른쪽에 여백을 두되, 최대 N 지점 이후로는
    # 실제로 그 이상의 N을 만드는 시나리오가 없으므로(F=0이 물리적으로 맞음) 값을 계속 이어그리지 않고
    # 최대 N 지점에서 y축 하한보다 훨씬 아래로 즉시 떨어뜨려, 그래프 밖으로 사라지는 형태로 그린다.
    # (로그 스케일 y축에서는 0 자체를 표시할 수 없어, "화면 밖으로 떨어짐"으로 F=0을 표현)
    x_lo = 1
    x_hi = max(n_values[-1] * 1.15, 2)

    finite_f = f_cum[f_cum > 0]
    y_bottom = finite_f.min() * 0.3 if len(finite_f) else 1e-8
    y_top = max(f_cum.max() * 3, 1e-2) if len(finite_f) else 1e-2
    near_zero = y_bottom * 1e-3  # y_bottom보다 훨씬 작아 ylim 밖으로 잘려 나가는 값

    # (N_max, F_max) → (N_max, near_zero) 수직 하강 → (x_hi, near_zero) 수평 연장(화면 밖이라 안 보임)
    n_plot = np.array([*n_values, n_values[-1], x_hi])
    f_plot = np.array([*f_cum, near_zero, near_zero])
    ax.step(n_plot, f_plot, where="post", color="tab:blue", linewidth=2)
    if show_markers:
        # 마커는 실제 계산된 지점(n_values, f_cum)에만 표시하고, 여백/하강 연장점에는 찍지 않는다.
        ax.plot(n_values, f_cum, linestyle="none", marker="o", markersize=3, color="tab:blue")
