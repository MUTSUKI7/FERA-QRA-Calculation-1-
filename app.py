...
with st.sidebar:
    ...
    show_grid = st.checkbox("위성사진 위에도 격자선 표시", value=False)
...
def draw_grid_overlay(img, grid_extent_m, cell_size_m, color=(255, 255, 255, 120)):
    """기존 이미지(위성사진 등) 위에 격자선을 겹쳐 그림"""
    W, H = img.size
    overlay = Image.new("RGBA", img.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    n_cells = int(round(grid_extent_m / cell_size_m))
    for i in range(n_cells + 1):
        x = int(round(i * W / n_cells))
        y = int(round(i * H / n_cells))
...
def render_thermal(excel_bytes, image_file, grid_extent_m, cell_size_m, canvas_px,
                    back_frac, cross_frac, show_grid=False):
    by_weather, wind_rose, fire_types, points_m = load_thermal_data(excel_bytes)

    if image_file is not None:
        img = Image.open(image_file).convert("RGB")
        if show_grid:
            img = draw_grid_overlay(img, grid_extent_m, cell_size_m)
    else:
...
def render_lsir(excel_bytes, image_file, grid_extent_m, cell_size_m, resolution_m,
                 back_frac, cross_frac, show_grid=False):
    ...
    if show_grid:
        n_cells = int(round(grid_extent_m / cell_size_m))
        grid_lines = np.linspace(0, grid_extent_m, n_cells + 1)
        for g in grid_lines:
            ax.axvline(g, color="white", linewidth=0.5, alpha=0.5, zorder=2)
            ax.axhline(g, color="white", linewidth=0.5, alpha=0.5, zorder=2)
...
if contour_type.startswith("Thermal"):
    try:
        result_img = render_thermal(EXCEL_BYTES, image_file, grid_extent_m, cell_size_m,
                                     canvas_px, back_frac, cross_frac, show_grid)
    ...
    if not run_lsir:
        ...
        try:
            with st.spinner("LSIR 필드 계산 중..."):
                fig, max_risk = render_lsir(EXCEL_BYTES, image_file, grid_extent_m, cell_size_m,
                                             grid_resolution_m, back_frac, cross_frac, show_grid)
            st.pyplot(fig)
