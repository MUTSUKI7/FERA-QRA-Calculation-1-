...
with st.sidebar:
    ...
    grid_cell_count = st.number_input(
        "격자 칸 수 (한 변당) — 범례에 칸 크기(m/cell) 표시용",
        min_value=1, value=20, step=1,
    )
    cell_size_m = grid_extent_m / grid_cell_count
...
def render_explosion():
    ...
    legend_draw.text(
        ...
        f"{grid_cell_count}x{grid_cell_count} cells).",
...
def compute_lsir_field(_excel_bytes, grid_extent_m, grid_res_m, back_frac, cross_frac):
    ...
    hole_col_phast = find_hole_size_col(
        phast_df, exclude_cols={"IS_ID", "Weather_Class"} | set(thermal_level_cols)
    )
    ...
    for _, row in phast_df.iterrows():
        ...
        if hs_val is None:
            continue
        key = (row["IS_ID"], hs_val, row["Weather_Class"])
    ...
    hole_col_ignition = find_hole_size_col(
        ignition_df, exclude_cols={"IS_ID", "P_jet", "P_flash", "P_vce", "P_pool"}
    )
    ...
    for _, row in ignition_df.iterrows():
        is_id, hs = row["IS_ID"], row[hole_col_ignition]
