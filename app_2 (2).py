...
def rect_band_fractions(x_min, x_max, y_min, y_max, center, r_severe, r_mild, bearing,
                         mode, back_frac, cross_frac, n_samples=12):
    """사각형 영역을 n_samples x n_samples 격자로 샘플링해, 심각(severe)/경미(mild) 밴드에
    걸치는 면적비를 근사 계산한다 (면적비 기반 정밀 판정)."""
...
def rect_single_fraction(x_min, x_max, y_min, y_max, center, r, bearing, mode,
                          back_frac, cross_frac, n_samples=12):
    """단일 밴드(예: Flash Fire LFL)에 대한 사각형-면적 겹침비를 근사 계산한다."""
    if r is None or r <= 0:
        return 0.0
    xs = np.linspace(x_min, x_max, n_samples)
    ys = np.linspace(y_min, y_max, n_samples)
...
def render_area_drawer():
    """위성사진 위에 Area 사각형을 직접 그리고, 이름을 붙이고, CSV로 저장/불러올 수 있는 UI."""
    st.subheader("🖊️ Area(구역) 직접 그리기")
    st.caption(
        "위성사진 위에 마우스로 사각형을 그리고, 아래 표에서 각 사각형에 이름을 붙여주세요. "
        "이름은 엑셀 Occupancy 시트의 구역(컬럼)명과 일치해야 인원수가 연결됩니다 "
    ...
    with col_up:
        uploaded_csv = st.file_uploader(
            "저장된 Area CSV 불러오기", type=["csv"], key="area_csv_uploader"
        )
        if uploaded_csv is not None:
            try:
                df_loaded = pd.read_csv(uploaded_csv)
                needed = {"Area_Name", "Loc_X_min", "Loc_X_max", "Loc_Y_min", "Loc_Y_max"}
                if not needed.issubset(set(df_loaded.columns)):
                    st.error(f"CSV에 필요한 컬럼이 없습니다: {sorted(needed)}")
                else:
                    st.session_state.drawn_areas = df_loaded.to_dict("records")
                    st.success(f"{len(df_loaded)}개 Area를 불러왔습니다.")
            except Exception as e:
    ...
    if not HAS_CANVAS:
        ...
        def px_rect_to_m(left, top, w, h):
            x_min = left / px_per_m_canvas
            x_max = (left + w) / px_per_m_canvas
            y_max = (canvas_size - top) / px_per_m_canvas
            y_min = (canvas_size - (top + h)) / px_per_m_canvas
        ...
        if canvas_result.json_data is not None:
            for obj in canvas_result.json_data.get("objects", []):
                ...
                new_rects_px.append((left, top, w, h))

        if st.button(f"캔버스에 그린 사각형 {len(new_rects_px)}개를 목록에 추가", key="add_drawn_rects"):
            count_before = len(st.session_state.drawn_areas)
            added = 0
            for i, (left, top, w, h) in enumerate(new_rects_px):
                if w <= 0 or h <= 0:
                    continue
                x_min, x_max, y_min, y_max = px_rect_to_m(left, top, w, h)
                st.session_state.drawn_areas.append({
                    "Area_Name": f"Area_{count_before + added + 1}",
                    "Loc_X_min": round(x_min, 1), "Loc_X_max": round(x_max, 1),
                    "Loc_Y_min": round(y_min, 1), "Loc_Y_max": round(y_max, 1),
                })
                added += 1
            if added:
                st.success(f"{added}개 사각형을 추가했습니다. 아래 표에서 이름을 수정해주세요.")
                st.rerun()
    ...
    for a in st.session_state.drawn_areas:
        try:
            ...
            if not name or x_max <= x_min or y_max <= y_min:
                continue
            areas_out.append({"name": name, "x_min": x_min, "x_max": x_max,
...
def compute_fn_curve(_excel_bytes, back_frac, cross_frac, areas_json):
    ...
    def accumulate_single_event(weight, is_id, center, r, bearing, mode, v):
        ...
        for a in matched_areas:
            if not a["occupancy"]:
                continue
            frac = rect_single_fraction(a["x_min"], a["x_max"], a["y_min"], a["y_max"],
