...
def render_area_drawer():
    ...
    st.caption(
        ...
        "② 이어서 반대쪽 대각선 모서리를 클릭하면 사각형 하나가 자동으로 목록에 추가됩니다. "
        "③ 아래 표에서 Area_Name을 Occupancy 시트의 구역(컬럼)명과 맞게 고치고, "
    ...
    if not HAS_CLICK:
        ...
        click_value = streamlit_image_coordinates(bg_img_resized, key="area_click_img")
    ...
    if st.session_state.drawn_areas:
        ...
        st.info("아직 지정한 Area가 없습니다. 위 위성사진을 두 번 클릭하거나 CSV를 불러와주세요.")
