...
with st.sidebar:
    ...
    with st.expander("고급 설정 — 로브(lobe) 형태 가정치 (열복사·LSIR에 적용)"):
        ...
        st.caption(
            ...
            "아래를 켜면 Pool fire는 AGA(1974)/Thomas(1963)/Moorhouse(1982)의 화염 기울기(flame tilt) "
            "상관식으로 IS_ID·기상조건(풍속)별 값을 자동 계산해 위 슬라이더 대신 사용합니다. "
            "(PHAST_Distances의 Pool_Diameter_m·Pool_BurnRate_kgs, Wind rose의 Wind_Speed_(m/s) 필요 — "
            "둘 중 하나라도 없으면 그 시나리오는 자동으로 위 슬라이더 값으로 대체됩니다.)"
        )
        use_physics_pool = st.checkbox(
            "Pool Fire에 유체역학 기반 로브 형태 적용 (AGA/Thomas/Moorhouse 모델)", value=True
        )
...
# ------------------------------------------------------------------
# 유체역학 기반 Pool Fire 화염 기울기(flame tilt) 모델
# AGA(1974)/Thomas(1963)/Moorhouse(1982)의 무차원 풍속(u*) 상관식을 사용해
# Pool fire 시나리오의 BACK_FRAC/CROSS_FRAC을 IS_ID·기상조건별로 계산한다.
# (Jet fire는 분출속도 데이터가 없어 여전히 고정 슬라이더 값을 사용한다.)
# ------------------------------------------------------------------
def pool_fire_tilt_deg(wind_speed_ms, burn_rate_kgs, pool_diameter_m,
                        rho_air=1.2, g=9.81):
    """AGA/Thomas(1963) cosine 모델.
    u* = U / (g * m'' * D / rho_air)^(1/3)  (무차원 풍속)
    cos(theta) = 1        (u* <= 1)
    cos(theta) = 1 / u*    (u* > 1)
    필요 데이터가 없거나 물리적으로 무효하면 None을 반환한다 (호출부에서 슬라이더 값으로 대체)."""
...
def tilt_to_lobe_fracs(theta_deg, back_max=0.85, back_min=0.05,
                        cross_max=0.55, cross_min=0.15):
    """화염 기울기 각(theta, 수직 기준 0~90도)을 로브(lobe) 타원의 BACK_FRAC/CROSS_FRAC으로
    변환한다. theta=0(무풍) -> 대칭에 가까운 최댓값, theta=90(강풍으로 완전히 누움) -> 최솟값.
    주의: theta 자체는 문헌 상관식에서 나온 값이지만, 이 theta->ellipse 변환식은 검증된 문헌값이
    아니라 두 물리적 극한(무풍=대칭, 강풍=풍하측으로 좁고 길게)을 만족하도록 설계한 근사다."""
    theta = math.radians(min(max(theta_deg, 0.0), 90.0))
    back_frac_ = back_min + (back_max - back_min) * math.cos(theta)
    cross_frac_ = cross_min + (cross_max - cross_min) * math.cos(theta) ** 0.5
    return back_frac_, cross_frac_


def get_wind_speed_by_class(wind_rose_df):
    """Wind rose(Meteorology) 시트에서 Weather_Class별 대표 풍속(m/s)을 뽑아낸다.
    같은 Weather_Class에 풍향별로 여러 행이 있어도 풍속 값은 원래 동일해야 정상이므로
    평균을 대표값으로 사용한다 (컬럼이 없으면 빈 dict를 반환 -> 이후 전부 슬라이더 값으로 대체됨)."""
    speed_col = next(
...
def resolve_pool_fracs(use_physics, wind_speed_ms, burn_rate_kgs, pool_diameter_m,
                        fallback_back, fallback_cross):
    """물리 모델 스위치가 켜져 있고 필요 데이터(풍속·연소율·풀 직경)가 전부 있으면
    AGA/Thomas 상관식으로 계산한 BACK_FRAC/CROSS_FRAC을, 아니면(스위치 꺼짐 또는 데이터 부족)
    사이드바 슬라이더 값을 그대로 반환한다."""
    if use_physics:
        theta = pool_fire_tilt_deg(wind_speed_ms, burn_rate_kgs, pool_diameter_m)
        if theta is not None:
            return tilt_to_lobe_fracs(theta)
...
def render_thermal():
    ...
    legend_draw.text((lx + 8, note_y), "Directional lobes per Wind rose (ASSUMED shape, not raw PHAST output):",
                      fill=(230, 230, 230, 255), font=FONT_SMALL)
    pool_note = "Pool fire=AGA/Thomas physics model" if use_physics_pool else "Pool fire=fixed slider value"
            ...
            for wr in wind_rows:
                ...
                if cls["has_pool"]:
                    ...
                    bf_pool, cf_pool = resolve_pool_fracs(
                        ...
                        d.get("Pool_BurnRate_kgs"), d.get("Pool_Diameter_m"),
                        back_frac, cross_frac,
                    )
